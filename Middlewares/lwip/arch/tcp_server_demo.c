...
void tcp_server_test(void)
{
	...
	lcd_clear(WHITE);	//清屏
	g_point_color=RED; 	//红色字体
	lcd_show_string(30,30,200,16,16,"Apollo STM32F4/F7",g_point_color);
	...
	while(res==0)
	{
		...
		if(tcp_server_flag&1<<6)//是否收到数据?
		{
			...
			lcd_show_string(30,210,lcddev.width-30,lcddev.height-210,16,tcp_server_recvbuf,RED);//显示接收到的数据			
		...
		if(tcp_server_flag&1<<5)//是否连接上?
		{
			...
			{ 
				sprintf((char*)tbuf,"Client IP:%d.%d.%d.%d",g_lwipdev.remoteip[0],g_lwipdev.remoteip[1],g_lwipdev.remoteip[2],g_lwipdev.remoteip[3]);//客户端IP
 				lcd_show_string(30,170,230,16,16,tbuf,RED);
				g_point_color=RED;
				lcd_show_string(30,190,lcddev.width-30,lcddev.height-190,16,"Receive Data:",g_point_color);//提示消息		
				g_point_color=BLUE;//蓝色字体
				connflag=1;//标记连接了
			} 
		}else if(connflag)
